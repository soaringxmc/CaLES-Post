...
plt.plot(dns.diagonal.y, dns.diagonal.u/utau, color='black', label='DNS')
for i in range(len(folders)):
  les = Duct(folders[i])
  les.read_stats()
  plt.plot(les.diagonal.z, les.diagonal.u/utau, label=f'$\Delta_z/h={les.dy:.3f}$')
...
plt.ylim([0, 25])
# plt.show()
plt.savefig(f"duc_U_diagonal_DSMAG.pdf", format='pdf', bbox_inches='tight')
# plt.close()